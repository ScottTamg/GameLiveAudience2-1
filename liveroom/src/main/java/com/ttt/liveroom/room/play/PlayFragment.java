...
//        lazyLoad();
    ...
    protected void destroyRoom() {
        if (mAnchorNetSubscription != null && !mAnchorNetSubscription.isUnsubscribed()) {
            mAnchorNetSubscription.unsubscribe();
        }
    ...
    private void showLMRequestDialog() {
        ...
        dialog.setMessageDialogListener(new ApplyLMDialog.MessageDialogListener() {
            ...
            public void onCommitClick(ApplyLMDialog dialog) {
                ...
                // 发送连麦请求
                applyMicRequestToServer(mLiveId, mAnchorId, mMLocalUserId, mMLocalUserNickName, loginInfo.getAvatar(), ApplyMicBean.LM_TYPE_AUDIO);
    ...
    @Override
    protected void getAudioVideoCallList(AudioVideoCallListRes audioVideoCallListRes) {
        // TODO: 2018/7/13 音视频连麦用户列表 当前类不需要实现
    }

    @Override
    protected void updateLmUserMuteState(LmDataBean bean) {
        // TODO: 2018/7/15 更新连麦用户静音状态 视频直播间不需要实现
    }
    @Override
    protected void dealApplyMicRequest(LmAgreeOrRefuseRes bean) {
        if (mLoginInfo.getUserId().equals(bean.getData().getUserId())
                && mLiveId.equals(bean.getData().getRoomId())) {
            if ("2".equals(bean.getData().getType())) {
                ...
                isCanApplayLm = false;
                mRoomImgbtnLm.setImageResource(R.drawable.lianmaizhong_dianji);
                mTvMicText.setText(getString(R.string.lming));
            } else if ("3".equals(bean.getData().getType())) {
                toastShort(getString(R.string.refuse_lm));
                mRoomImgbtnLm.setImageResource(R.drawable.shenqinglianmai_dianji);
    ...
    protected void dealDisConnectLm(String userId) {
        if (mLoginInfo.getUserId().equals(userId)) {
            mRoomImgbtnLm.setImageResource(R.drawable.shenqinglianmai_dianji);
            mTvMicText.setText(getString(R.string.apply_connect_mic));
            if (mRoomLiveHelp != null) {
                mRoomLiveHelp.exitRoom();
...
//    }
    ...
    @Override
    public void toFollow(UserInfo userInfo, boolean isStat) {
        if (isStat) {
            presenter.unStarUser(loginInfo.getUserId(), userInfo.getId());
        } else {

            presenter.starUser(loginInfo.getUserId(), userInfo.getId());
        }
    }
...
//                        }
    ...
    public void onDisconnected(int errorCode) {
        ...
        if (errorCode == com.wushuangtech.library.Constants.ERROR_KICK_BY_MASTER_EXIT) {
            ((RoomActivity) getActivity()).showRoomEndInfoDialog(0, "0", "0");
        } else {
    ...
    public void onLoading() {
        if (loadingView != null) {
            ...
            if (getContext()!=null&&getContext().getApplicationContext()!=null){
                Glide.with(getContext().getApplicationContext()).load(R.drawable.loading).asGif().into(loadingView);
            }
...
//        }
    ...
    @Override
    public void startHostResult(String roomid) {
        toastShort(getString(R.string.follow_sucess));
        if (roomid.equals(mAnchorId)) {
            toptabstart.setVisibility(View.GONE);
        }
//        if (statAnchor) {
//            toptabstart.setVisibility(View.GONE);
