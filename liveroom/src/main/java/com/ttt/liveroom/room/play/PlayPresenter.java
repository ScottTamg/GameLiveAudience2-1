...
public class PlayPresenter extends BasePresenter<PlayerUiInterface> {
     ...
     * @param userIdFollow
     */
    public void starUser(String uid, final String userIdFollow) {
        Subscription subscription = NetManager.getInstance().create(RoomApi.class).starUser(uid, userIdFollow)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseObserver<BaseResponse<Object>>(getUiInterface()) {
                    @Override
                    public void onSuccess(BaseResponse<Object> response) {
                        if ("0".equals(response.getCode())) {
                            getUiInterface().startHostResult(userIdFollow);
                        }
                    }
                });
        addSubscription(subscription);
     ...
     * @param userIdFollow
     */
    public void unStarUser(String uid, final String userIdFollow) {
        Subscription subscription = NetManager.getInstance().create(RoomApi.class).unStarUser(uid, userIdFollow)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseObserver<BaseResponse<Object>>(getUiInterface()) {
                    @Override
                    public void onSuccess(BaseResponse<Object> response) {
                        if ("0".equals(response.getCode())) {
                            getUiInterface().unStarUser(userIdFollow);
                        }
                    }
                });
        addSubscription(subscription);
