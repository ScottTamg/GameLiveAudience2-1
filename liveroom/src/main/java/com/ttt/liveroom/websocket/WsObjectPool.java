...
import com.ttt.liveroom.bean.websocket.AudioVideoCallListReq;
...
import com.ttt.liveroom.bean.websocket.MuteReq;
...
public class WsObjectPool {
    ...
    private static final int REQ_AUDIO_VIDEO_CALL_LIST = 13;
    private static final int REQ_MUTE = 14;
    /**
     * 释放所有资源，清空数据。
     */
...
//        requestArray.put(APPLY_MIC_RESPONE, applyMicBeanResponse);
        ...
        //音视频连麦用户列表
        AudioVideoCallListReq audioVideoCallListReq = new AudioVideoCallListReq();
        audioVideoCallListReq.setMessageType(SocketConstants.AUDIO_VIDEO_CALL_LIST_REQ);
        audioVideoCallListReq.setData(new AudioVideoCallListReq.DataBean());
        requestArray.put(REQ_AUDIO_VIDEO_CALL_LIST, audioVideoCallListReq);

        //静音
        MuteReq muteReq = new MuteReq();
        muteReq.setMessageType(SocketConstants.MUTE_REQ);
        muteReq.setData(new MuteReq.DataBean());
        requestArray.put(REQ_MUTE, muteReq);
    }
...
//        UserInfo userInfo = DataManager.getInstance().getmUserInfo();
     ...
     */
    public static ApplyMicBean newApplyMicRequest(Context context, String liveId, String hostId, String id, String nickname, String avatar, String lmType) {
        checkInitOrThrow(context);
        ...
        request.getData().setAvatar(avatar);
        request.getData().setLmType(lmType);
        return request;
    }
...
//    }
     ...
     * @return
     */
    public static AudioVideoCallListReq newAudioVideoCallListReq(Context context, String roomId,
                                                                 String userId) {
        checkInitOrThrow(context);
        AudioVideoCallListReq request = get(REQ_AUDIO_VIDEO_CALL_LIST);
        request.getData().setRoomId(roomId);
        request.getData().setUserId(userId);
        return request;
    }
     ...
     * @return
     */
    public static MuteReq newMuteReq(Context context, String adminId, String roomId, String userId,
                                     String mute) {
        checkInitOrThrow(context);
        MuteReq request = get(REQ_MUTE);
        request.getData().setAdminUserId(adminId);
        request.getData().setMute(mute);
        request.getData().setRoomId(roomId);
        request.getData().setUserId(userId);
        return request;
    }
